...
#target_nn.build(input_shape=(None,num_features))
#target_nn.load_weights("Duling_DQN_TF_MODEL.h5")
...
for episode in range(num_episodes+1):
  ...
  if (epsilon < 0.2) and (np.mean(last_100_ep_rewards) > min_reward):
      main_nn.save_weights("Duling_DQN_TF_MODEL_1.h5")
...
plt.plot(rew)
plt.show()
np.save("LOSS_VALUE_Duling_DQN_2.npy", loss_value, allow_pickle=True)
np.save("Reward_Duling_DQN_2.npy", rew, allow_pickle=True)
np.save("Epsilon_Duling_DQN_2.npy", espi, allow_pickle=True) 
