...
main_nn.load_weights("TF_DQN_MODEL.h5")
...
for episode in range(num_episodes+1):
  ...
  if (epsilon < 0.2) and (np.mean(last_100_ep_rewards) > min_reward):
      main_nn.save_weights("TF_DQN_MODEL_1.h5")
...
plt.plot(rew)
plt.show()
#np.save("LOSS_VALUE_TF_DQN_2.npy", loss_value, allow_pickle=True)
np.save("Reward_TF_DQN_straight.npy", rew, allow_pickle=True) 
np.save("Epsilon_TF_DQN_straight.npy", espi, allow_pickle=True) 
